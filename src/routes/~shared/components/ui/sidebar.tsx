...
import {
  ...
  AvatarFallback,
  AvatarImage,
} from '@/routes/~shared/components/ui/avatar';
...
import {
  ...
  SheetHeader,
  SheetTitle,
} from '@/routes/~shared/components/ui/sheet';
import { Skeleton } from '@/routes/~shared/components/ui/skeleton';
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
} from '@/routes/~shared/components/ui/tooltip';
import { useIsMobile } from '@/routes/~shared/hooks/use-mobile';
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      {...props}
    >
      <SidebarSimpleIcon className="cn-rtl-flip" />
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive ? 'true' : undefined}
      className={cn(
        'text-sidebar-foreground/50 ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground [&>svg]:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:text-sidebar-accent-foreground h-7 gap-2 rounded-md px-2 focus-visible:ring-2 data-[size=md]:text-sm data-[size=sm]:text-xs [&>svg]:size-4 flex min-w-0 -translate-x-px items-center overflow-hidden outline-hidden group-data-[collapsible=icon]:hidden disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:shrink-0',
        className,
      )}
      {...props}
    />
...
function SidebarCorpChoosing({
  corps,
  selectedCorp,
  onCorpChange,
}: {
  corps: { name: string; plan: string; logo?: string }[];
  selectedCorp: { name: string; plan: string; logo?: string };
  onCorpChange?: (corp: { name: string; plan: string; logo?: string }) => void;
}) {
          ...
          <DropdownMenuTrigger asChild>
            ...
            >
              <div className="bg-sidebar-primary text-sidebar-primary-foreground flex aspect-square size-8 items-center justify-center rounded-lg">
                {selectedCorp.logo ? (
                  <img
                    src={selectedCorp.logo}
                    alt={selectedCorp.name}
                    className="size-4"
                  />
                ) : (
                  <span className="text-xs font-semibold">
                    {selectedCorp.name.charAt(0)}
                  </span>
                )}
              </div>
              <div className="grid flex-1 text-left text-sm leading-tight group-data-[collapsible=icon]:hidden">
                ...
                <span className="truncate text-xs">{selectedCorp.plan}</span>
              </div>
...
function SidebarAccountManagement({
  user,
}: {
